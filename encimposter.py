...
def Main():
    ...
    + space)
    path = Write.Input('Tên File Cần Encode => ', colors, interval)
    path = os.path.normpath(path)
    if not os.path.exists(path):
        Colorate.Error('Không Thấy File')
        return
    if not os.path.isfile(path):
        Colorate.Error('Not a file.')
        return
    Write.Input('[!] NHẤN XUỐNG DÒNG ĐỂ TẠO FILE...', Col.DynamicMIX([Col.orange, Col.yellow]), interval)
    with open(path, 'r', encoding = 'utf-8') as file:
        code = file.read()
    code = Impostor.Obfuscate(code)
    filename = os.path.basename(path).removesuffix('.py')
    with open(filename + '_obf.py', 'a+', encoding = 'utf-8') as file:
        file.write(code)
    Write.Print(f'[!] ENCODE THÀNH CÔNG !', Col.green, interval)
    input()

if __name__ == '__main__':
    System.Title('Impostor - by NAMTOOL')
    System.Size(140, 40)
    Intro()