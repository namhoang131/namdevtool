        ...
        try:
            ...
            try:
                if response.json()['status_code'] == 0:
                    _lock.acquire()
                    print(Colorate.Horizontal(Colors.green_to_blue, f'[Nam~Tool]| BUFF VIEW THÀNH CÔNG: {success} views'))
                    success += 1
                    _lock.release()
            except:
...
def fetch_proxies():
    ...
    for url in url_list :
        ...
        if response.ok:
            with open("namtool/proxies.txt", "a+") as f:
...
if __name__ == "__main__":
    with open('namtool/devices.txt', 'r') as f:
        devices = f.read().splitlines()
    
    with open('namtool/config.json', 'r') as f:
        config = json.load(f)
    ...
    if config['proxy']['use-proxy']:
        with open('namtool/proxies.txt', 'r') as f:
    ...
    txt = """
            ...
            \033[1;36m╰─────────────────────────────────────────────────────⋟─╯ 
            \033[1;31m                TOOL BUFF VIEW TIKTOK
            \033[1;36m╰─────────────────────────────────────────────────────⋟─╯"""
    print(
        (txt)
        )
