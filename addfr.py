...
def check_live(cookie):
	...
	try:
		...
		return f'\033[1;37mFacebook: {vang}{name} {do}| {trang}UserID: {vang}{user_id}'
	except:
		return False

cookie = input(f'\033[1;37mVui Lòng Nhập Cookie Facebook: {vang}')

if check_live(cookie) == False:
	exit(f'\033[1;31mCookie Facebook Die Hoặc Bị Văng !!')
else:
...
while True:
	...
	for add in find:
		...
		print(f'[{dem}] | \033[1;37mĐANG KẾT BẠN VỚI {do}| \033[1;33m{name_} {do}- {lam}{id} ')
		sleep(5)