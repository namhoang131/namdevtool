#Tool Made By Nguyễn Hoàng Nam
#Chúc các bạn dùng tool vui vẻ ^^
#Mở đầu Tool
...
class API_PRO5_ByNgDucPhat:
    def banner(self):
        ...
        banner = """
            ...
            \033[1;36m╰─────────────────────────────────────────────────────⋟─╯ 
            \033[1;31m    TOOL REG PAGE PROFILE UP AVT
            \033[1;36m╰─────────────────────────────────────────────────────⋟─╯"""
        print(banner)
    def ndp_delay_tool(self, p):
        while(p>1):
            p=p-1
            print(f'{do}[  {vang}Vui Lòng Chờ {do}]{do}[{lam}|{do}]{tim}[LO......][{p}]','     ',end='\r');sleep(1/6)
            print(f'{do}[ {xam} Vui Lòng Chờ {do}]{do}[{black}/{do}]{luc}[LOA.....][{p}]','     ',end='\r');sleep(1/6)
            print(f'{do}[ {hong} Vui Lòng Chờ {do}]{do}[{tim}-{do}]{vang}[LOAD....][{p}]','     ',end='\r');sleep(1/6)
            print(f'{do}[  {luc}Vui Lòng Chờ{do}]{do}[{luc}+{do}]{xduong}[LOADI...][{p}]','     ',end='\r');sleep(1/6)
            print(f'{do}[  {lam}Vui Lòng Chờ {do}]{do}[{xam}\{do}]{trang}[LOADIN..][{p}]','     ',end='\r');sleep(1/6)
            print(f'{do}[  {trang}Vui Lòng Chờ {do}][{xduong}|{do}]{xam}[LOADING.][{p}]','     ',end='\r');sleep(1/6)
    def getthongtinfacebook(self, cookie: str):
        ...
        try:
            print(f'{xduong}Đang Tiến Hành Check Live', end="\r")
    ...
    def RegPage(self, cookie, name, uid, fb_dtsg, jazoest):
        ...
        try:
            ...
            print(f'{do}Reg Thất Bại Có Vẻ Acc Của Bạn Đã Bị Block!!')
            return False
# =========================== [ START TOOL ] ===========================
dpcutevcl = API_PRO5_ByNgDucPhat()
dpcutevcl.banner()
print(f'{xanhlam}[ENTER - ĐỂ DỪNG NHẬP]')
while True:
    stt+=1
    cookie_fb = input(f'{trang}VUI LÒNG NHẬP COOKIE THỨ [{stt}]: {vang}')
    if cookie_fb == '':
        break
    checklive = dpcutevcl.getthongtinfacebook(cookie_fb)
    if checklive != False:
        print(f'{trang}Name Facebook: '+checklive[0])
...
print('─'*50)
luachon = input(f'{trang}BẠN MUỐN REG PAGE XONG UP AVT KHÔNG? [Y/N]: {vang}')
print('─'*50)
print('[ENTER - ĐỂ DỪNG NHẬP]')
while True:
    stt2+=1 
    link_img = input(f'{trang}VUI LÒNG NHẬP LINK ẢNH THỨ [{stt2}]: {vang}')
    if link_img == '':
        break
    list_img.append(link_img)
print('─'*50)
slpage = int(input('\033[1;37mBẠN MUỐN TẠO BAO NHIÊU PAGE THÌ DỪNG TOOL: \033[1;33m'))
print('─'*50)
delay = int(input('\033[1;37mVUI LÒNG NHẬP DELAY REG PAGE: \033[1;33m '))
print('─'*50)
# Tiến Hành Chạy Tool
dpcutevcl.banner()
print('─'*50)
print(f'{lam}Đã Tìm Thấy: '+str(len(list_clone))+' Cookie')
print(f'{lam}Đã Tìm Thấy: '+str(len(list_img))+' Link Image')
print('─'*50)
while True:
    for dulieuclone in list_clone:
        ...
        if luachon == 'Y' or luachon == 'y':
            ...
            if dpcutevcl != False:
                print(f' {luc}╰─> UP_AVT_SUCCESS | [UID PAGE: {idpage}]')
            else:
                print(f' {do}╰─> UP_AVT_ERROR | [UID PAGE: {idpage}]')
        dpcutevcl.ndp_delay_tool(delay)
        if dem == slpage:
            input(f'{luc}Done {dem}, {lam}Page </> ENTER ĐỂ EXIT')
            exit()