...
def berserk(content: str, key: int) -> str:
    ...
    _final_content_ = fr"""class NamDev():
...
def main():
    ...
    file = Write.Input("File Cần Encode -> ", Colors.red_to_yellow, interval=0.005)

    if not file.strip() or not isfile(file):
        Colorate.Error("File Không Tồn Tại!")
        return
    ...
    file = file.removesuffix(".py") + "_obf.py" 
    ...
    print(Colorate.Diagonal(Colors.red_to_yellow, f"""
Using key {key}...
