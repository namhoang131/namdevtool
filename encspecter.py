...
try:
    if (
        __author__ != "NamDev" or
        __OBFUSCATION_BY__ != "NAMDEV" or
        __ENCODE__ != "Specter!" or
        "Specter" not in globals() or
        "Func" not in globals()
    ):
        int('skid')
except:
    input("!/bin/python3\n\nOBFUSCATION: NamDev\nDate OBF:%s") % time.ctime
    __import__('sys').exit()    
...
def main():
    ...
    file = input(stage(f"File Cần Encode {Col.blue}-> {Col.reset}", "?")).replace('"','').replace("'","")
    print('\n')
