...
def get_static_api_content(api_urls):
    ...
    for api_url in api_urls:
        ...
        if response.status_code == 200:
            ...
            print(f"{Fore.GREEN}Scan Proxy từ API -> https://apiv1.dagtri.org/proxy/maybiluaroi{Fore.RESET}")
        else:
            print(f"{Fore.RED}Failed Scan Proxy từ API -> https://apiv1.dagtri.org/proxy/maybiluaroi{Fore.RESET}")
    return api_content
    ...
    if proxy_list:
        ...
        with open(output_path, 'w') as f:
            for proxy in proxy_list:
                ...
                f.write(proxy + "\n")
        print(f"{Fore.GREEN}Đã lưu file proxy thành công{Fore.RESET}")
    else:
        print(f"{Fore.RED}Không có proxy nào trong file cả{Fore.RESET}")
