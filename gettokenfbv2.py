...
def input_cookie():
    print(f"""
            ...
            \033[1;36m╰─────────────────────────────────────────────────────⋟─╯ 
            \033[1;31m             TOOL GET TOKEN FACEBOOK CÁC LOẠI
            \033[1;36m╰─────────────────────────────────────────────────────⋟─╯
""")
    print('%sNẾU A2F MỞ, NHẬP ĐƯỜNG LINK BÊN DƯỚI ĐỂ LẤY MÃ A2F'%(P))
    print('%shttps://business.facebook.com/business_locations\n'%(P))
    cookie = input('%sNhập Cookie Facebook : %s'%(P,J))
...
def req_info_token(cooki,token):
    try:
        ...
        return('%sQuyền Không Có Sẵn%s'%(M,P))
...
class specific_token:
    ...
    def get_kode(self,act):
        ...
        try:
            ...
            print('Đăng Nhập Thất Bại => %s'%(act))
    ...
    def req_info_token(self,tok):
        try:
            ...
            except Exception as e: crf = '%sQuyền Không Có Sẵn%s'%(M,P)
            print('')
            print('%s[ %s ]%s'%(P,gtx,P))
            print('%s%s%s'%(J,tok,P))
            print('%s%s%s'%(H,crf,P))
